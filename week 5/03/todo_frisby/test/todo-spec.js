...
/*this should clear all the lists in the API and return success, lists deleted*/

  frisby.create('delete all lists')
  .delete('http://localhost:8081/lists')
  .expectStatus(200)
  .expectHeaderContains('Content-Type', 'application/json')
  .expectJSON({status: 'success', message: 'lists deleted'})
  .toss()
...
frisby.create('add a new list')
  .post('http://localhost:8081/lists', {"name": "shopping", "list": ["Cheese", "Bread", "Butter"]}, {json: true})
  .expectStatus(201)
  .expectJSONTypes({
    "status": String,
    "message": String,
